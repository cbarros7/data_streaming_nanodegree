...
systemctl start confluent-kafka-rest >  startup.log 2>&1
systemctl start confluent-kafka-connect >  startup.log 2>&1
systemctl start confluent-ksql >  startup.log 2>&1
sed -i 's/md5/trust/g' /etc/postgresql/10/main/pg_hba.conf
/etc/init.d/postgresql start > startup.log 2>&1
su postgres -c "createuser root -s" > startup.log 2>&1
createdb classroom > startup.log 2>&1
psql -d classroom -c "DROP TABLE IF EXISTS purchases; CREATE TABLE purchases(id INT PRIMARY KEY, username VARCHAR(100), currency VARCHAR(10), amount INT);" > startup.log 2>&1
...
kafka-topics --delete  --bootstrap-server localhost:9092 --topic com.udacity.streams.purchases > startup.log 2>&1
kafka-topics --create  --bootstrap-server localhost:9092 --topic com.udacity.streams.users --replication-factor 1 --partitions 10 > startup.log 2>&1
kafka-topics --create  --bootstrap-server localhost:9092 --topic com.udacity.streams.purchases --replication-factor 1 --partitions 10 > startup.log 2>&1
kafka-topics --delete  --bootstrap-server localhost:9092 --topic com.udacity.streams.pages > startup.log 2>&1
kafka-topics --delete  --bootstrap-server localhost:9092 --topic com.udacity.streams.clickevents > startup.log 2>&1
kafka-topics --create  --bootstrap-server localhost:9092 --topic com.udacity.streams.pages --replication-factor 1 --partitions 10 > startup.log 2>&1
kafka-topics --create  --bootstrap-server localhost:9092 --topic com.udacity.streams.clickevents --replication-factor 1 --partitions 10 > startup.log 2>&1
